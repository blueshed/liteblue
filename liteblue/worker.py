...
from .handlers.broadcast_mixin import RedisBroadcaster
...
class Worker:
    ...
    async def shutdown(self, wait: bool = True):
        handlers.BroadcastMixin._broadcaster_ = None
        if self.redis:
            self.redis.close()
    ...
    async def subscribe(self, work_queue: str):
        ...
        try:
            while True:
                document = await subscription.blpop(
                    work_queue, encoding='utf-8'
                )
                ...
                if future:
                    if 'error' in result:
                        future.set_exception(
                            RemoteException(result.get('error'))
                        )
                    else:
